...
export default function ProjectFacilitiesPage() {
  ...
  const handlePreview = async (e) => {
    ...
    try {
      const res = await projectFacilitiesService.parseFacilityExcel(file, selectedProject);
      if (res.success) {
        setPreview(res.data);
      } else {
        setError(res.message || 'Failed to parse facilities excel.');
      }
    } catch (err) {
  ...
  const handleGenerateBatch = async () => {
    ...
    try {
      const data = {
        ...
        facilities: preview.validFacilities,
        batchName: batchName
      };

      const res = await projectFacilitiesService.generateBatch(data);
      if (res.success) {
        navigate('/inspection-engine'); // Navigate to main inspection engine where batches are listed
      } else {
        setError(res.message || 'Failed to create project facilities batch.');
      }
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto w-full">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Inspect and rate Bus Bay and Truck Lay By assets using uploaded facility chainages.</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1">
                <label className="block text-sm font-medium text-gray-700 mb-1">Upload Facilities Excel</label>
        ...
        {preview && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {/* Validation Table for Valid Facilities */}
            <div className="mb-6 overflow-x-auto">
              <h4 className="text-md font-bold text-gray-800 mb-2">Parsed Facilities</h4>
              <table className="w-full text-sm text-left text-gray-500 border">
                ...
                <tbody>
                  {preview.validFacilities.map((f, idx) => (
                  ...
                  {preview.validFacilities.length === 0 && (
                    <tr>
                      <td colSpan="6" className="px-4 py-4 text-center">No valid facilities found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            {/* Invalid Records */}
            {preview.invalidFacilities.length > 0 && (
              <div className="mb-6 overflow-x-auto">
