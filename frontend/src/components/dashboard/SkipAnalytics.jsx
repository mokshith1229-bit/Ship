          ...
          <div className="fixed inset-0 z-[60] bg-gray-50 flex flex-col overflow-hidden">
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm shrink-0">
              <div className="flex items-center gap-3">
                <MdImage className="text-[#5cb85c] text-2xl" />
                <h2 className="text-xl font-bold text-gray-800">
                  Image Gallery {selectedProject && <span className="ml-2 text-sm bg-green-50 text-green-700 border border-green-200 px-2 py-1 rounded">{selectedProject}</span>}
                </h2>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {(!data?.recentSkips || data.recentSkips.length === 0) ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400">
                  <MdImage className="text-6xl mb-4 opacity-20" />
                  ...
                  {data.recentSkips.map((skip, idx) => (
                    ...
                    >
                      ...
                      <div className="relative z-10 p-4 w-full translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                        <div className="flex justify-between items-start mb-1">
                          <span className="bg-white/20 backdrop-blur-md text-white text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-wide border border-white/30">
                            {skip.assetType || 'Unknown'}
                          </span>
                          <span className="text-xs text-white/70 font-medium">
                            {new Date(skip.timestamp || skip.updatedAt).toLocaleDateString()}
                          </span>
                        </div>
                        <h4 className="text-white font-bold text-sm truncate mb-1" title={skip.reason}>
                          {skip.reason || 'No Reason Provided'}
                        </h4>
                        <div className="flex items-center gap-1 text-white/80 text-[11px] truncate">
                          <MdOutlineLocationOn className="text-white/60" /> {skip.chainage || 'N/A'}
                        </div>
                      </div>
                    </motion.div>
                  ))}
